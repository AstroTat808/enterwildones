...
export async function createMagicLink(user){if(!secret())throw new Error('Passport session secret is not configured.');const tokenId=randomBytes(24).toString('base64url'),expiresAt=new Date(Date.now()+MAGIC_TTL*1000);const db=getDatabase();await db.sql`INSERT INTO wildones_passport_magic_links(token_id,user_id,email,expires_at) VALUES(${tokenId},${user.userId},${user.email},${expiresAt.toISOString()})`;const payload=encode({v:1,typ:'wildones-passport-magic',tokenId,userId:user.userId,email:user.email,exp:Math.floor(expiresAt.getTime()/1000)});return `${payload}.${sign(payload)}`;}
export async function consumeMagicLink(token=''){if(!secret()||!String(token).includes('.'))return null;const[p,s]=String(token).split('.',2);if(!safeEqual(s,sign(p)))return null;let data;try{data=decode(p);}catch{return null;}if(data.v!==1||data.typ!=='wildones-passport-magic'||Number(data.exp)<=Math.floor(Date.now()/1000))return null;const db=getDatabase(),client=await db.pool.connect();try{await client.query('BEGIN');const result=await client.query('SELECT token_id,user_id,email,expires_at,consumed_at FROM wildones_passport_magic_links WHERE token_id=$1 FOR UPDATE',[data.tokenId]);const row=result.rows[0];if(!row||row.consumed_at||new Date(row.expires_at).getTime()<=Date.now()||row.user_id!==data.userId||row.email!==data.email){await client.query('ROLLBACK');return null;}await client.query('UPDATE wildones_passport_magic_links SET consumed_at=NOW() WHERE token_id=$1',[data.tokenId]);await client.query('COMMIT');return data;}catch(error){await client.query('ROLLBACK').catch(()=>{});throw error;}finally{client.release();}}
