...
export default async (req) => {
  ...
  if (Number(active.failures) >= MAX_FAILURES) {
    await writeAudit({ action: 'admin.login_rate_limited', actor: 'anonymous', targetType: 'session', detail: { ipHash: rate.ipHash } });
    return json({ error: 'Too many failed admin login attempts. Try again later.' }, 429, { 'Retry-After': String(Math.max(60, Math.ceil((Number(active.windowStart) + WINDOW_MS - now) / 1000))) });
  }
  ...
  if (!supplied || !equal(supplied, expected)) {
    ...
    await writeAudit({ action: 'admin.login_failed', actor: 'anonymous', targetType: 'session', detail: { ipHash: rate.ipHash } });
    return json({ error: 'Invalid admin key.' }, 401);
  }
  await rate.store.delete(rate.key).catch(() => {});
  await writeAudit({ action: 'admin.login_succeeded', actor: 'admin', targetType: 'session', detail: { ipHash: rate.ipHash } });
